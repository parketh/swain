...
- **Root cause (corrected by a faithful local replay probe).** The initial context-growth/compaction theory was **falsified**. Replaying make-mips's exact stalling request (141 messages, ~68K tokens) against live glm-5.2 with no idle watchdog:
  - **Single-threaded, glm answers in ~2–8s** (fast time-to-first-token). The context size / TTFT is not the problem, and auto-compaction is not the lever.
  - **At `high` reasoning effort, glm-5.2 stochastically "runs away" reasoning** — one observed turn produced 53,717 events over 587s of *continuous* streaming (max inter-event gap ~5s, so the idle watchdog never trips). ~2 of 6 sampled requests ran away for many minutes. This burns the eval's 1800s wall clock and explains the `path-tracing` timeout; the idle-timeout raise does not help it (the gaps are small; total duration is the issue).
  - The eval's exact `no data for 120s` idle stall (make-mips) was **not** reproduced — an intermittent provider-side drop, likely aggravated by running `n-concurrent=4`.

**Decision: KEEP (as general robustness), but it is not the lever for these failures.** 60→120s idle + exponential backoff is still a correct, no-regression reliability improvement for genuinely transient drops. It does not flip a pass because the dominant failure is glm's runaway per-turn reasoning (and intermittent load-induced stalls), not idle silence. The next iteration targets **bounding glm's per-turn generation** (max-output cap / lower default reasoning effort / per-turn wall cap) and measuring at lower concurrency — tracked separately.
