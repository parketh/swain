...
import { compactSession, defaultTokenCounter, isValidlyPaired, selectCut } from "../src/context"
...
describe("compaction", () => {
  test("manual compaction with no prior summary summarizes the prefix", async () => {
  ...
  test("a second compaction updates the one summary instead of stacking", async () => {
    const state = session(transcript())
    ...
    expect(compactionBlocks(state.messages)).toHaveLength(1)
    expect(state.compaction.summary).toBe(summaryText)
  })

  test("kept transcript begins at an assistant boundary and stays validly paired", async () => {
    const state = session(transcript())
    await run(state, { reason: "manual", tailBudget: 20 })

    // messages[0] is the compaction meta (user); the kept tail begins after it.
