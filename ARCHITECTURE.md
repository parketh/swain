...
- **Standalone packaging:** release builds (`scripts/build-release.ts`) cross-compile `bin/swain.tsx` into a single Bun executable and ship it alongside a private static `rg` at `libexec/rg`. Because release artifacts must not depend on `rg` being discoverable via `PATH`, `sidecars.ts` derives `../libexec/rg` from `process.execPath` and exports it as `SWAIN_RG_PATH` at startup; `@swain/core`'s ripgrep runner honors that variable (falling back to `rg` on `PATH` for source runs). An explicit `SWAIN_RG_PATH` always wins and is never overwritten, and the sidecar is never added to `PATH`.

## evals

A dependency-locked Python project (`evals/`, `uv`) that runs Swain as a custom installed agent against Terminal-Bench 2 and DeepSWE. Written up in `evals/README.md`; the native-trace → ATIF conversion contract is in `specs/0012-harbor-evals.md`.

### Key decisions

- **Native trace → ATIF boundary:** Swain owns an opt-in native trace bundle (`swain exec --trace-dir`): a versioned (`schemaVersion: 1`) root snapshot plus one file per completed child, holding committed messages, reasoning, tool calls/results, timestamps, durations, and per-response usage — serializable facts only, no model functions or caches. The Python side is the *only* place that knows ATIF: `evals/common/trajectory.py` converts the native bundle into ATIF-v1.7 dictionaries (one system step for the prompt, one agent step per assistant response with the following tool-result folded into its observation, subagents linked by external `subagents/<id>.json` path). Each file is an independently valid trajectory; the wrapper sums root + child token totals into the framework `AgentContext` exactly once. Cost/cached-token fields are omitted because Swain has no authoritative values.
- **Two separate wrappers, shared core:** Harbor and Pier expose different install/network APIs, so `evals/harbor/agent.py` and `evals/pier/agent.py` each inherit their own framework's `BaseInstalledAgent` and never import each other. All reusable logic — artifact install shell, provider/credential/domain table, redaction, and the ATIF converter — lives in `evals/common/` so endpoints and install steps are never duplicated. Pier additionally declares a `network_allowlist()` of exactly the selected provider's domain (DeepSWE runs network-isolated).
- **Artifact dependency, not source builds:** the adapters install an exact, checksummed Swain **release** (the `linux-x64-{glibc,musl}` archives from the release workflow) into read-only `/opt/swain/v<version>`; they never build from source or install a host-local binary inside a task. Routing is off — the single Harbor/Pier `model_name` drives the parent and every subagent — and only the selected provider's key reaches the process: the wrapper reads exactly that key from the host environment (rejecting base-URL/gateway overrides via `select_agent_env`) and forwards it through the framework's scoped agent-env, never on the command line or a base-class-logged `env=`. Known limitation: on Docker environments, Harbor/Pier deliver env into the container via `docker compose exec -e KEY=VALUE`, so the value still appears briefly in that per-exec subprocess argv on the host — unavoidable in Harbor 0.20 / Pier 0.3 (no env-file/stdin path for `docker exec`). This is a transient per-exec spike, not the long-lived `harbor run`/`pier run` argv exposure that sourcing from the host env eliminates; non-Docker environments (e.g. Pier `--env modal`) are unaffected.